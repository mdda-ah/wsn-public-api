	...
	function get($f3) {
		...
		if ($f3->get('sensor_type_id') == null) {
			...
			$f3->set(
				'result',
				$db->exec(
					'select coordcounter, datetime, device_id, reading, nodecounter, sensor_id from readings where device_id=:device_id and sensor_id=:sensor_type_id and sensor_id in (select sensor_type_id from sensors where sensors.available=1 and sensors.device_id=readings.device_id) order by datetime desc limit 0,:database_query_limit',
					array (
						':device_id' => $f3->get('PARAMS.id'),
						':database_query_limit' => $f3->get('database_query_limit'),
